-- Resolve stuck subjects fd6672c6-02c4-455e-a4d9-91e4ae9d308c and 765d2ab0-1b55-4701-babd-af5247442de5
...
UPDATE privacy_export_requests
SET status = 'RESOLVED',
    notes = 'Resolved stuck subject with orphaned dependency task 168a6edf-9643-4775-a637-7c1803624025'
WHERE subject_id = '765d2ab0-1b55-4701-babd-af5247442de5'
  AND status IN ('PENDING', 'PROCESSING');

UPDATE privacy_erasure_requests
SET status = 'RESOLVED',
    reason = 'Resolved stuck subject with orphaned dependency task 168a6edf-9643-4775-a637-7c1803624025'
WHERE subject_id = '765d2ab0-1b55-4701-babd-af5247442de5'
  AND status IN ('PENDING', 'PROCESSING');
