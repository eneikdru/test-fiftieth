...
public interface DossierReportRepository extends JpaRepository<DossierReport, Long> {
    ...
    @Modifying
    @Query("UPDATE DossierReport d SET d.status = :newStatus, d.summaryText = :summaryText, d.downloadUrl = :downloadUrl WHERE d.id = :id AND d.status = :oldStatus")
    int updateStatusAndDetails(@Param("id") Long id, @Param("oldStatus") String oldStatus, @Param("newStatus") String newStatus, @Param("summaryText") String summaryText, @Param("downloadUrl") String downloadUrl);
