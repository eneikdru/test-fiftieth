...
@SpringBootTest
@Transactional
class RuntimeContractVerificationV20260824053157128Test {
    ...
    @Test
    @DisplayName("Given mandatory Flyway migration V20260824053157128, When executed against datastore, Then patch completes cleanly and resolves pending privacy requests")
    void testRuntimeContractPatchMigrationExecutesCleanly() {
        ...
        jdbcTemplate.update(
            ...
            "test-exp-128", subjectId, "PENDING", "JSON", "Initial pending export request"
        );
        jdbcTemplate.update(
            ...
            "test-era-128", subjectId, "PROCESSING", "tok-128", "FULL", "Initial processing erasure request"
        );
        ...
        assertDoesNotThrow(() -> {
            ...
            try {
                ScriptUtils.executeSqlScript(
                    conn,
                    new EncodedResource(new ClassPathResource("db/migration/V20260824053157128__align_datastore_runtime_contract.sql")),
                    false,
                    false,
        ...
        String exportStatus = jdbcTemplate.queryForObject(
            "SELECT status FROM privacy_export_requests WHERE request_id = ?", String.class, "test-exp-128"
        );
        String erasureStatus = jdbcTemplate.queryForObject(
            "SELECT status FROM privacy_erasure_requests WHERE request_id = ?", String.class, "test-era-128"
        );
