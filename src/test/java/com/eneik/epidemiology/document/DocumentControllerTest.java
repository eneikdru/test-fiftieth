    ...
    void testDownloadDocument_NotFound_Returns404() throws Exception {
        mockMvc.perform(get("/api/v1/documents/999999/download")
                ...
                .andExpect(result -> {
                    ...
                    org.junit.jupiter.api.Assertions.assertTrue(body == null || body.isEmpty() || body.contains("\"status\":404") || body.contains("Not Found") || body.contains("Document not found"), "Body should be empty or standard error, not generated byte content");
                    org.junit.jupiter.api.Assertions.assertFalse(body != null && body.contains("Содержимое документа"), "Body should NOT contain fallback byte content");
                });
    }
