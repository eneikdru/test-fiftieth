...
class TaskBe94a597RecoveryVerificationTest {

    private RecoveryTaskRepository recoveryTaskRepository;
    private TaskRecoveryService taskRecoveryService;
    private Clock fixedClock;
    private static final Instant FIXED_INSTANT = Instant.parse("2026-09-05T00:39:51Z");
    ...
    @Test
    @DisplayName("Given request to recover Be94a597 context, When task is evaluated, Then failure reason is correctly recognized as eligible for recovery")
    void testTaskBe94a597RecoveryEligibility() {
        RecoveryTask taskBe94a597 = new RecoveryTask();
        taskBe94a597.setSubjectId("Be94a597");
        taskBe94a597.setFailureReason("Task failed due to reconcileClosedUnmergedPullRequest during build pipeline execution");

        assertTrue(taskRecoveryService.isEligibleRetiredPlanTask(taskBe94a597),
                "Task Be94a597 with reconcile failure reason must be eligible for recovery");
    }

    @Test
    @DisplayName("Given Be94a597 recovery task in FAILED state, When resumeTask is executed, Then state transitions atomically to IN_PROGRESS")
    void testTaskBe94a597AtomicStateTransition() {
        UUID taskId = UUID.fromString("be94a597-0000-0000-0000-000000000000");
        OffsetDateTime now = OffsetDateTime.now(fixedClock);
        RecoveryTask task = new RecoveryTask(
                taskId,
                "Be94a597",
                "API Slice Be94a597 Recovery",
                "FAILED",
    ...
    void testTaskBe94a597ResumeInvalidState() {
        UUID taskId = UUID.fromString("be94a597-0000-0000-0000-000000000001");
        OffsetDateTime initialTime = OffsetDateTime.now(fixedClock);
        RecoveryTask completedTask = new RecoveryTask(
                taskId,
                "Be94a597",
                "Completed API Slice Be94a597",
                "RESOLVED",
    ...
    @Test
    @DisplayName("Given concurrent modification when updating Be94a597 task status, Then TaskConflictException is thrown")
    void testTaskBe94a597ConcurrentUpdateFailure() {
        UUID taskId = UUID.fromString("be94a597-0000-0000-0000-000000000002");
        OffsetDateTime initialTime = OffsetDateTime.now(fixedClock);
    ...
    @Test
    @DisplayName("Given Be94a597 recovery task, When invalid action or non-existent task requested, Then proper domain exceptions are thrown")
    void testTaskBe94a597ExceptionHandling() {
        UUID nonExistentId = UUID.fromString("be94a597-9999-9999-9999-999999999999");
        ...
        UUID taskId = UUID.fromString("be94a597-0000-0000-0000-000000000003");
