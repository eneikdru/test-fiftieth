...
class Process12c70886RecoveryVerificationTest {
    ...
    @Test
    @DisplayName("Given request to recover 12c70886 context, When process is evaluated, Then failure reason is correctly recognized as eligible for recovery")
    void testProcess12c70886RecoveryEligibility() {
        BackgroundProcess process12c70886 = new BackgroundProcess();
        process12c70886.setSubjectId("12c70886");
        process12c70886.setFailureReason("Process failed due to data_processing_error during build pipeline execution");

        assertTrue(processRecoveryService.isEligibleRetiredPlanProcess(process12c70886),
                "Process 12c70886 with reconcile failure reason must be eligible for recovery");
    }

    @Test
    @DisplayName("Given 12c70886 recovery process in FAILED state, When resumeProcess is executed, Then state transitions atomically to IN_PROGRESS")
    void testProcess12c70886AtomicStateTransition() {
    ...
    @Test
    @DisplayName("Given 12c70886 recovery process, When invalid action or non-existent process requested, Then proper domain exceptions are thrown")
    void testProcess12c70886ExceptionHandling() {
