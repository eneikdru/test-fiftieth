...
class Process289c8597RecoveryVerificationTest {

    private BackgroundProcessRepository recoveryProcessRepository;
    private ProcessRecoveryService processRecoveryService;
    private Clock fixedClock;
    private static final Instant FIXED_INSTANT = Instant.parse("2026-09-03T07:31:00Z");
    ...
    @Test
    @DisplayName("Given request to recover 289c8597 context, When process is evaluated, Then failure reason is correctly recognized as eligible for recovery")
    void testProcess289c8597RecoveryEligibility() {
        BackgroundProcess process289c8597 = new BackgroundProcess();
        process289c8597.setSubjectId("289c8597");
        process289c8597.setFailureReason("Process failed due to data_processing_error during build pipeline execution");

        assertTrue(processRecoveryService.isEligibleRetiredPlanProcess(process289c8597),
                "Process 289c8597 with reconcile failure reason must be eligible for recovery");
    }

    @Test
    @DisplayName("Given 289c8597 recovery process in FAILED state, When resumeProcess is executed, Then state transitions atomically to IN_PROGRESS")
    void testProcess289c8597AtomicStateTransition() {
        UUID processId = UUID.fromString("289c8597-0000-0000-0000-000000000000");
        OffsetDateTime now = OffsetDateTime.now(fixedClock);
        BackgroundProcess process = new BackgroundProcess(
                processId,
                "289c8597",
                "API Slice 289c8597 Recovery",
                "FAILED",
    ...
    void testProcess289c8597ResumeInvalidState() {
        UUID processId = UUID.fromString("289c8597-0000-0000-0000-000000000001");
        OffsetDateTime initialTime = OffsetDateTime.now(fixedClock);
        BackgroundProcess completedProcess = new BackgroundProcess(
                processId,
                "289c8597",
                "Completed API Slice 289c8597",
                "RESOLVED",
    ...
    @Test
    @DisplayName("Given concurrent modification when updating 289c8597 process status, Then ProcessConflictException is thrown")
    void testProcess289c8597ConcurrentUpdateFailure() {
        UUID processId = UUID.fromString("289c8597-0000-0000-0000-000000000002");
        OffsetDateTime initialTime = OffsetDateTime.now(fixedClock);
    ...
    @Test
    @DisplayName("Given 289c8597 recovery process, When invalid action or non-existent process requested, Then proper domain exceptions are thrown")
    void testProcess289c8597ExceptionHandling() {
        UUID nonExistentId = UUID.fromString("289c8597-9999-9999-9999-999999999999");
        ...
        UUID processId = UUID.fromString("289c8597-0000-0000-0000-000000000003");
