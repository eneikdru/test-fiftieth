...
class TaskFe4db1dbRecoveryVerificationTest {

    private RecoveryTaskRepository recoveryTaskRepository;
    private TaskRecoveryService taskRecoveryService;
    private Clock fixedClock;
    private static final Instant FIXED_INSTANT = Instant.parse("2026-09-09T02:00:00Z");
    ...
    @Test
    @DisplayName("Given request to recover Fe4db1db context, When task is evaluated, Then failure reason is correctly recognized as eligible for recovery")
    void testTaskFe4db1dbRecoveryEligibility() {
        RecoveryTask taskFe4db1db = new RecoveryTask();
        taskFe4db1db.setSubjectId("Fe4db1db");
        taskFe4db1db.setFailureReason("Task failed due to reconcileClosedUnmergedPullRequest during build pipeline execution");

        assertTrue(taskRecoveryService.isEligibleRetiredPlanTask(taskFe4db1db),
                "Task Fe4db1db with reconcile failure reason must be eligible for recovery");
    }

    @Test
    @DisplayName("Given Fe4db1db recovery task in FAILED state, When resumeTask is executed, Then state transitions atomically to IN_PROGRESS")
    void testTaskFe4db1dbAtomicStateTransition() {
        UUID taskId = UUID.fromString("fe4db1db-0000-0000-0000-000000000000");
        OffsetDateTime now = OffsetDateTime.now(fixedClock);
        RecoveryTask task = new RecoveryTask(
                taskId,
                "Fe4db1db",
                "API Slice Fe4db1db Recovery",
                "FAILED",
    ...
    void testTaskFe4db1dbResumeInvalidState() {
        UUID taskId = UUID.fromString("fe4db1db-0000-0000-0000-000000000001");
        OffsetDateTime initialTime = OffsetDateTime.now(fixedClock);
        RecoveryTask completedTask = new RecoveryTask(
                taskId,
                "Fe4db1db",
                "Completed API Slice Fe4db1db",
                "RESOLVED",
    ...
    @Test
    @DisplayName("Given concurrent modification when updating Fe4db1db task status, Then TaskConflictException is thrown")
    void testTaskFe4db1dbConcurrentUpdateFailure() {
        UUID taskId = UUID.fromString("fe4db1db-0000-0000-0000-000000000002");
        OffsetDateTime initialTime = OffsetDateTime.now(fixedClock);
    ...
    @Test
    @DisplayName("Given Fe4db1db recovery task, When invalid action or non-existent task requested, Then proper domain exceptions are thrown")
    void testTaskFe4db1dbExceptionHandling() {
        UUID nonExistentId = UUID.fromString("fe4db1db-9999-9999-9999-999999999999");
        ...
        UUID taskId = UUID.fromString("fe4db1db-0000-0000-0000-000000000003");
