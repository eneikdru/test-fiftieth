...
class Task289c8597RecoveryVerificationTest {
    ...
    @Test
    @DisplayName("Given request to recover 289c8597 context, When task is evaluated, Then failure reason is correctly recognized as eligible for recovery")
    void testTask289c8597RecoveryEligibility() {
        RecoveryTask task289c8597 = new RecoveryTask();
        task289c8597.setSubjectId("289c8597");
        task289c8597.setFailureReason("Task failed due to reconcileClosedUnmergedPullRequest during build pipeline execution");

        assertTrue(taskRecoveryService.isEligibleRetiredPlanTask(task289c8597),
                "Task 289c8597 with reconcile failure reason must be eligible for recovery");
    }

    @Test
    @DisplayName("Given 289c8597 recovery task in FAILED state, When resumeTask is executed, Then state transitions atomically to IN_PROGRESS")
    void testTask289c8597AtomicStateTransition() {
        UUID taskId = UUID.fromString("289c8597-0000-0000-0000-000000000000");
    ...
    void testTask289c8597ResumeInvalidState() {
        UUID taskId = UUID.fromString("289c8597-0000-0000-0000-000000000001");
    ...
    @Test
    @DisplayName("Given concurrent modification when updating 289c8597 task status, Then TaskConflictException is thrown")
    void testTask289c8597ConcurrentUpdateFailure() {
        UUID taskId = UUID.fromString("289c8597-0000-0000-0000-000000000002");
        OffsetDateTime initialTime = OffsetDateTime.now(fixedClock);
    ...
    @Test
    @DisplayName("Given 289c8597 recovery task, When invalid action or non-existent task requested, Then proper domain exceptions are thrown")
    void testTask289c8597ExceptionHandling() {
        ...
        UUID taskId = UUID.fromString("289c8597-0000-0000-0000-000000000003");
